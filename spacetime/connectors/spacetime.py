...
from applications.search.crawler_frame import get_analytics
...
class ObjectlessSpacetimeConnection(object):
    ...
    def get_crawler_stats(self, dataframe_change):
        if "stats" in dataframe_change:
            ...
            subdomains = analytics[0]
            page_stats = analytics[1]
            cutoffCount = str(dataframe_change['stats'][0])
            if int(cutoffCount) >= 3000:
                with open('report.txt', 'a') as the_file:
                    the_file.write('Page with most out links is ' + page_stats[0] + '\n')
                    the_file.write('It contains ' + str(page_stats[1]) + ' links\n\n\n')
                    the_file.write('Summary of subdomains visited\n\n')
                    for key in subdomains:
                        the_file.write(key + ': ' + str(subdomains[key]) + '\n')
                print ('Nice job, you just hit 3000 url download, crawler will exit now!')
                exit()
